...
#### 2017 - 4-6:  code works on data analysis examples and a multivariate data import
...
residual.boot <- function(formula, B=1000, seed=NULL){
...
################NOT USED CODE
#  resp <- eval(as.name(all.vars(formula)))
#  resp <- resp[,dim(resp)[2]]
#  

#Bootstrap response?
    #boot.env$bootResp <- resp - obsDataResid + bootResid       #bootstrap response 
